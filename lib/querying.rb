...
def select_series_title_with_most_human_characters
  "SELECT series.title, COUNT(characters.species) AS num_species FROM series JOIN characters ON series.id = characters.series_id WHERE characters = 'Human' GROUP BY series.title ORDER BY num_species DESC LIMIT 1;"
end
